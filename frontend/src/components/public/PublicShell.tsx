  ...
  return <MotionConfig reducedMotion="user"><div className="bhon-public">
    ...
    <header className={'public-header' + (scrolled ? ' is-scrolled' : '')}>
      <div className="public-container public-nav">
        ...
        <div className="public-nav-actions"><a href="/login">Entrar <ArrowUpRight size={14} /></a><a className="public-button small" href="/comece">Começar teste <ArrowUpRight size={15} /></a></div>
    ...
    <dialog ref={dialog} id="public-navigation" className="public-mobile-menu" aria-label="Navegação" onCancel={event => { event.preventDefault(); close(); }} onKeyDown={event => { if (event.key === 'Escape') { event.preventDefault(); close(); } }}>
      ...
      <nav>{links.map(([name, href], i) => <a key={name} href={href} onClick={close}><small>0{i + 1}</small>{name}</a>)}<a href="/login">Entrar <ArrowUpRight /></a><a className="public-button" href="/comece">Começar teste</a></nav>
    </dialog>
    <main id="public-main">{children}</main>
    <footer className="public-footer">
      <div className="public-container">
        <div className="public-footer-top"><div><p className="public-kicker">VAMOS CONVERSAR</p><h2>Sua próxima fase<br />começa com uma conversa.</h2></div><a className="public-contact" href="mailto:bhonsuport@gmail.com">bhonsuport@gmail.com <ArrowUpRight /></a></div>
