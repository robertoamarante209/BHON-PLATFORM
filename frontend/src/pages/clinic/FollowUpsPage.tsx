...
export const FollowUpsPage: React.FC = () => {
  ...
  const canContact = hasClinicPermission(currentUser, 'recovery.contact');
  const canManage = hasClinicPermission(currentUser, 'recovery.manage');
  const canAct = canContact || canManage;
  ...
  const openFollowUp = (followUp: FollowUp) => {
    setSelectedFollowUp(followUp); setAction(canContact ? 'LOG_CONTACT' : 'COMPLETE'); setNotes(''); setOutcome(''); setNewDeadline('');
  ...
  const submitAction = async (event: React.FormEvent) => {
    ...
    if ((action === 'LOG_CONTACT' && !canContact) || (action !== 'LOG_CONTACT' && !canManage)) {
      setError('Seu acesso não permite executar esta ação.'); return;
    }
